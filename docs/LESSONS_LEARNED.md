...
**Provenance: this file was carried over verbatim from `Arcade-Psikyo_MiSTer`, a completed
MiSTer arcade core built by the same author against the same toolchain, hardware and
framework.** It is not a status file for this project and nothing in it describes Fuuki
hardware. It is the accumulated cost of finding each of these the hard way once, and the
entire reason it is here is so that this core does not pay for them a second time.

How to use it:

- **Read the section headings before starting a new subsystem**, not after it misbehaves.
  Most entries are cheap to obey up front and expensive to retrofit.
- Entries referencing Psikyo-specific module names (`tilemap_line_engine`, `psikyo_top`,
  `spriteram_dbuf`, ...) still state a *general* rule; the module name is evidence, not scope.
- Cross-project rules that already bind decisions in this core are cited from
  `docs/ROADMAP.md` by section name.
- When this core finds something genuinely new, append it here with the same shape the
  existing entries use: **the rule, the mechanism that made the wrong assumption plausible,
  and the evidence that settled it.** Mark new entries `[Fuuki]` so the two projects'
  findings stay distinguishable.
...
| Instantiate TG68K.C or T80 | "CPU cores (TG68K.C, T80)" |
...
| Add a debug probe | "Debug instrumentation: how not to fool yourself" |
