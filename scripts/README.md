...
| `cfg.py` | set OSD status bits in a per-core `.CFG` by read-modify-write, so untouched bits survive; the CFG is only read when the core loads. The debug bits (layer masks, trace controls, `marker`, `no_fm`/`no_pcm`) only reach the core in the `Fuuki_stp` build; the release forces them off |
