...
--
-- Driven by scripts/mame_capture.py. Parameters are environment variables
-- because an autoboot script gets no arguments:
--
...
--
-- Everything is read through the CPU program space, so the files hold what
-- the CPU would read, device handlers included.
...
local function dump(addr, len, name)
    ...
    -- Word reads: byte reads through a 16-bit handler can differ.
    local buf = {}
    for i = 0, len - 2, 2 do
...
-- FG-3's sprite tile bank: fuukifg3.cpp maps 0xa00000 writeonly(), so a
-- read returns 0, and install_write_tap on that range aborts the script at
-- load (MAME then runs without capturing). Read the driver's :tilebank share
-- instead, every frame into a ring: the register is delayed two frames with
-- the sprite data, so the value in force is the one from two frames earlier.
local tb_ring = {}
...
-- Written once, at capture time. Writing the file every frame stopped the
-- notifier reaching its capture body; keep per-frame work to arithmetic.
...
-- Optional vreg write log. Register 0x1c (the next raster-interrupt line) is
-- logged with each write so writes can be placed on the scanline timeline.
local vreglog
if VREGLOG then
...
-- KEEP THE SUBSCRIPTION ALIVE. If the object returned by
-- emu.add_machine_frame_notifier is garbage collected, the callback silently
-- stops firing: early frames capture, late ones produce nothing, exit status 0.
-- An error inside a notifier raises a MAME dialog and never reaches run.lua's
-- pcall, so record it here and exit rather than leave a partial capture.
...
local function frame_body()
    ...
    -- The reference the RTL's rendering of this state is compared against.
    mach.video:snapshot()
